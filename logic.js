...
async function displayFundamentalAnalysisForm(container, stockSymbol, exchange){
    const response = await fetch(`https://cf0ebb89-401f-4c17-93ed-ce66790377f8-00-21i5m8412bdrv.sisko.replit.dev/fundamental?stk=${stockSymbol}&exc=${exchange}`);
...
async function technical_analysis() {
    ...
    if (selectedImage) {
        ...
        const response = await fetch(`https://cf0ebb89-401f-4c17-93ed-ce66790377f8-00-21i5m8412bdrv.sisko.replit.dev/technical?stk=${stockSymbol}&exc=${exchange}&tmf=${timeFrame}`, {
            method: 'POST',
            body: formData
        });
        const result = await response.text();
...
async function displayIntradayForm(container, stockSymbol, exchange) {
    const response = await fetch(`https://cf0ebb89-401f-4c17-93ed-ce66790377f8-00-21i5m8412bdrv.sisko.replit.dev/positions?stk=${stockSymbol}&exc=${exchange}&pmt=intra`);
    const result = await response.text();
    container.innerHTML = `<div>${result}</div>`;
}

async function displaySwingForm(container, stockSymbol, exchange) {
    const response = await fetch(`https://cf0ebb89-401f-4c17-93ed-ce66790377f8-00-21i5m8412bdrv.sisko.replit.dev/positions?stk=${stockSymbol}&exc=${exchange}&pmt=swing`);
    const result = await response.text();
    container.innerHTML = `<div>${result}</div>`;
}
