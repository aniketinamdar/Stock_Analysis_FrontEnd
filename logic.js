...
async function displayFundamentalAnalysisForm(container, stockSymbol, exchange){
    const response = await fetch(`https://b7289841-7ac3-4107-9724-2ea88b9073a8-00-3kwnfu5jof51h.worf.replit.dev:4000/fundamental?stk=${stockSymbol}&exc=${exchange}`);
...
async function technical_analysis() {
    ...
    if (selectedImage) {
        ...
        const response = await fetch(`https://b7289841-7ac3-4107-9724-2ea88b9073a8-00-3kwnfu5jof51h.worf.replit.dev:4000/technical?stk=${stockSymbol}&exc=${exchange}&tmf=${timeFrame}`, {
            method: 'POST',
            body: formData
        });
        const result = await response.text();
...
async function displayIntradayForm(container, stockSymbol, exchange) {
    const response = await fetch(`https://b7289841-7ac3-4107-9724-2ea88b9073a8-00-3kwnfu5jof51h.worf.replit.dev:4000/positions?stk=${stockSymbol}&exc=${exchange}&pmt=intra`);
    const result = await response.text();
    container.innerHTML = `<div>${result}</div>`;
}

async function displaySwingForm(container, stockSymbol, exchange) {
    const response = await fetch(`https://b7289841-7ac3-4107-9724-2ea88b9073a8-00-3kwnfu5jof51h.worf.replit.dev:4000/positions?stk=${stockSymbol}&exc=${exchange}&pmt=swing`);
    const result = await response.text();
    container.innerHTML = `<div>${result}</div>`;
}
